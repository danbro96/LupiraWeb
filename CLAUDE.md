...
- **Backend** — [LupiraWeb.Server/](LupiraWeb.Server/). .NET 10 minimal API, a **stateless BFF** over [LupiraCareerApi](https://github.com/danbro96/LupiraFamilyWeb). No database, no Marten, no EF — it reads career/résumé data from CareerApi over HTTP via a typed client ([Integration/CareerApi/](LupiraWeb.Server/Integration/CareerApi/)) with owner-bearer auth, and maps to public response DTOs. The DTO-supporting enums + `Location` live in [LupiraWeb.Server/Contracts/](LupiraWeb.Server/Contracts/); there are no event-sourced aggregates here. `Microsoft.AspNetCore.OpenApi` emits `/openapi/v1.json` at runtime and, via `Microsoft.Extensions.ApiDescription.Server`, writes `lupiraweb.client/backend-openapi.json` at `dotnet build`. Scalar UI mounted at `/scalar/v1` in Development. Health probes: `/livez` + `/readyz` (readiness pings CareerApi).
- **Project layout** — `.NET` projects are flat at the repo root: [LupiraWeb.Server/](LupiraWeb.Server/) and [LupiraWeb.Server.Tests/](LupiraWeb.Server.Tests/), wired in [LupiraWeb.slnx](LupiraWeb.slnx).
